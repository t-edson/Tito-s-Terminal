...
//  res.used:=false;  //No hay obligación de que la asignación devuelva un valor.
  if Upcase(vars[p1.ivar].nom) = 'TIMEOUT' then begin
    //variable interna
    config.fcMacros.TpoMax := p2.GetValInt;
    config.fcConex.UpdateChanges;  //actualiza
  end else if Upcase(vars[p1.ivar].nom) = 'curPORT' then begin
    //variable interna
    config.fcConex.Port := IntToStr(p2.GetValInt);
    config.fcConex.UpdateChanges;  //actualiza
  end;
end;
...
begin
  ...
  //  res.used:=false;  //No hay obligación de que la asignación devuelva un valor.
  if Upcase(vars[p1.ivar].nom) = 'CURIP' then begin
    //variable interna
    config.fcConex.IP := p2.GetValStr;
    config.fcConex.UpdateChanges;  //actualiza
  end else if Upcase(vars[p1.ivar].nom) = 'CURTYPE' then begin
    //variable interna
    case UpCase(p2.GetValStr) of
    ...
    config.fcConex.UpdateChanges;  //actualiza
  end else if Upcase(vars[p1.ivar].nom) = 'CURENDLINE' then begin
    //variable interna
    if UpCase(p2.GetValStr) = 'CRLF' then
      config.fcConex.SendCRLF := true
    else
      config.fcConex.SendCRLF := false;
    config.fcConex.UpdateChanges;  //actualiza
  end else if Upcase(vars[p1.ivar].nom) = 'CURAPP' then begin
    //indica aplicativo actual
    config.fcConex.Other := p2.GetValStr;
    config.fcConex.UpdateChanges;  //actualiza
  end else if Upcase(vars[p1.ivar].nom) = 'PROMPTSTART' then begin
    //indica aplicativo actual
    config.fcDetPrompt.prIni:= p2.GetValStr;
    config.fcDetPrompt.detecPrompt := true;  //por defecto
    config.fcDetPrompt.ConfigCambios;  //actualiza
  end else if Upcase(vars[p1.ivar].nom) = 'PROMPTEND' then begin
    //indica aplicativo actual
    config.fcDetPrompt.prFin:= p2.GetValStr;
    config.fcDetPrompt.detecPrompt := true;  //por defecto
    config.fcDetPrompt.ConfigCambios;  //actualiza
  end;
end;
...
procedure bol_procLoad(var Op: TOperand);
begin
  //carga el operando en res
  setRes(tipStr,Op.estOp);
...
//  res.used:=false;  //No hay obligación de que la asignación devuelva un valor.
  if Upcase(vars[p1.ivar].nom) = 'PROMPTDETECT' then begin
    //variable interna
    config.fcDetPrompt.detecPrompt := p2.GetValBool;
    config.fcDetPrompt.ConfigCambios;  //actualiza
  end;
end;
...
begin
  ...
  if Perr.HayError then exit;
  msgbox(stack[sp].cons.valStr);  //sabemos que debe ser String
...
begin
  PopResult;  //saca parámetro 1
  if Perr.HayError then exit;
  msgbox(IntToStr(stack[sp].cons.valInt));  //sabemos que debe ser Entero
  //el tipo devuelto lo fijará el framework, al tipo definido
  res.estOp:=STORED_LIT;  //el valor devuelto no importa, pero debe devolver algo
end;
procedure fun_disconnect;
...
procedure fun_connectSSH;
//conecta con SSH
var
  conAct: TfraConexion;
begin
  PopResult;  //saca parámetro 1
  frmPrincipal.InicConectSSH(stack[sp].cons.valStr);   //inicia conexión
  res.estOp:=STORED_LIT;  //el valor devuelto no importa, pero debe devolver algo
end;
procedure fun_sendln;
//desconecta la conexión actual
var
  lin: String;
begin
...
begin
  PopResult;  //saca parámetro 1
  if frmPrincipal.proc = nil then exit;
  n10mil := stack[sp].cons.valInt * 100;
  //lazo de espera
  tic := 0;
